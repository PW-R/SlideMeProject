...
import { useRouteInfo } from "./PositionContext";

//  สร้างเส้นทาง วาง Marker
const LeafletGeocoder = ({ origin, destination }) => {
  ...
  useEffect(() => {
    if (
      ...
      L.marker([origin.position.lat, origin.position.lng], { icon: originIcon })
        .addTo(map)
        .bindPopup("ตำแหน่งต้นทาง");

      L.marker([destination.position.lat, destination.position.lng], {
        icon: customIcon,
      })
        .addTo(map)
        .bindPopup("ตำแหน่งปลายทาง");
